...
# з 2 по  40 строку відбувається створення інтерфейсу
import os
...
from PyQt5.QtCore import Qt  # нужна константа Qt.KeepAspectRatio для 
# изменения размеров с сохранением пропорций
...
class ImageProcessor():
   ...
   def showImage(self, path):
       lb_image.hide()   #приховуємо область де появиться фото
       pixmapimage = QPixmap(path) 
       w, h = lb_image.width(), lb_image.height()  # беремо координати області 
                                                   # де буде фотографія
       pixmapimage = pixmapimage.scaled(w, h, Qt.KeepAspectRatio) # адаптуємо розміри фото 
                                                                  # під наші розміри
       lb_image.setPixmap(pixmapimage) 
       lb_image.show()   # робимо фотографію видимою
